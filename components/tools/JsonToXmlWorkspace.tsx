...
function jsonToXml(data: unknown, indent = 0): string {
  const pad = "  ".repeat(indent);
  if (data === null) return `${pad}<null/>`;
  if (typeof data === "boolean" || typeof data === "number") return `${String(data)}`;
  if (typeof data === "string") return data.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
  if (Array.isArray(data)) {
    return data.map((item) => {
      const inner = jsonToXml(item, indent + 1);
      return `${pad}  <item>${typeof item === "object" && item !== null ? `\n${inner}\n${pad}  ` : inner}</item>`;
    }).join("\n");
  }
  if (typeof data === "object") {
    return Object.entries(data as Record<string, unknown>).map(([k, v]) => {
      ...
      return `${pad}<${tag}>${jsonToXml(v, indent)}</${tag}>`;
    }).join("\n");
  }
  return String(data);
}
...
function xmlHighlight(xml: string): string {
  return xml
    .replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;")
    .replace(/&lt;\?([^?]*)?\?&gt;/g, (m) => `<span style="color:var(--text-secondary,#888)">${m}</span>`)
    .replace(/&lt;\/([a-zA-Z0-9_.-]+)&gt;/g, `&lt;<span style="color:var(--json-key,#3291ff)">/$1</span>&gt;`)
    .replace(/&lt;([a-zA-Z0-9_.-]+)\/&gt;/g, `&lt;<span style="color:var(--json-key,#3291ff)">$1</span>/&gt;`)
    .replace(/&lt;([a-zA-Z0-9_.-]+)&gt;/g, `&lt;<span style="color:var(--json-key,#3291ff)">$1</span>&gt;`);
}
...
export default function JsonToXmlWorkspace({ fileLabel = "output.xml", inputLabel = "JSON Input", outputLabel = "XML Output", outputId = "json-to-xml-output" }: Props) {
  ...
  function actionConvert() {
    ...
    const xml = convert(jsonRef.current);
    setRawOutput(xml); setOutputHtml(xmlHighlight(xml)); setStatus({ text: "Converted", type: "success" });
  }

  function actionClear() { setInputVal(""); setOutputHtml(""); setRawOutput(""); jsonRef.current = null; setStatus({ text: "Ready", type: "default" }); }

  function loadSample() {
    const s = { name: "JSONPro", version: "1.0", active: true, features: ["format", "validate"], meta: { author: "Dev", year: 2024 } };
    const str = JSON.stringify(s, null, 2); setInputVal(str); jsonRef.current = s; setStatus({ text: "Valid JSON", type: "success" });
    const xml = convert(s); setRawOutput(xml); setOutputHtml(xmlHighlight(xml));
  }
  ...
  function downloadOutput() {
    if (rawOutput) { const b = new Blob([rawOutput], { type: "application/xml" }); const u = URL.createObjectURL(b); const a = document.createElement("a"); a.href = u; a.download = "data.xml"; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(u); }
  }
        ...
        <div className="workspace-toolbar-inline" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 20px", borderBottom: "1px solid var(--border,#262626)", flexWrap: "wrap", gap: 12 }}>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
            {(["Sample", "Clear"] as const).map((label) => (
              <button key={label} className="app-btn-hover" onClick={label === "Sample" ? loadSample : actionClear} style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 12px", background: "transparent", color: "var(--text-secondary,#888)", border: "1px solid var(--border,#262626)", borderRadius: 6, fontSize: "0.8rem", fontWeight: 500, cursor: "pointer", transition: "0.2s" }}>{label}</button>
            ))}
